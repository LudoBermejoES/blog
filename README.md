...
The 12 default hero covers that came with the theme have been **removed**. They
were synthetic cyberpunk art with no recognizable footage, likeness, or mark in
them, but their upstream licensing was undocumented — the one asset class left
that could not be accounted for. No post referenced them, so nothing on the site
changed when they went.

`src/assets/blog/default-covers/` is therefore empty by default. The auto-assign
mechanism still works: drop images in and the scaffolder starts picking from
them again. Without it, `npm run new-post` simply omits `heroImage` and you
supply your own cover per post.
...
If default covers exist in `src/assets/blog/default-covers/`, a stable cover is auto-assigned by slug hash (you can replace `heroImage` later). The directory ships empty, so by default the `heroImage` line is omitted and you add your own.
