...
async function main() {
  ...
  const themeDep = pkg?.dependencies?.['@cyberdream/astro-theme'];
  ...
  const linkPath = path.join(cwd, 'node_modules', '@cyberdream', 'astro-theme');
  ...
  try {
    ...
    fail('missing node_modules/@cyberdream/astro-theme. Run: npm install');
  }

  if (!stat.isSymbolicLink()) {
    fail('node_modules/@cyberdream/astro-theme is not a symlink. Run: npm install');
  }
  ...
  if (resolvedLink !== resolvedExpected) {
    fail(
      `node_modules/@cyberdream/astro-theme points to ${resolvedLink}, expected ${resolvedExpected}. Run: npm install`
    );
  }
